...
func AuthenticateRequest(request *http.Request, secretCode string) (*AccessDetails, rest_errors.RestErr) {
	...
	tokenAuth, err := extractTokenMetadata(request, secretCode)
	if err != nil {
		return nil, rest_errors.NewUnauthorizedError("unauthorized")
	}
...
func verifyToken(r *http.Request, secretCode string) (*jwt.Token, error) {
...
func tokenValid(r *http.Request, secretCode string) error {
	token, err := verifyToken(r, secretCode)
...
func extractTokenMetadata(r *http.Request, secretCode string) (*AccessDetails, error) {
	token, err := verifyToken(r, secretCode)
	if err != nil {
		return nil, err
	}
